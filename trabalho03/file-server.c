...
#define COD_OK3_APPEND "OK-2 method CREATE OK\n"
#define COD_OK4_APPEND "OK-3 method CREATE OK\n"
...
#define COD_ERROR_3_FILE "ERROR-3 File not found\n"
...
void create_file(int sockfd, req_t r){
    int fd = open((char *)(r.filename), O_RDONLY, S_IRUSR);
    if(fd == -1){
        FILE *fp  = fopen ((char *)(r.filename), "w");
        fclose (fp);
        close(fd);
        return;
    }
...
void append_to_file(int sockfd, req_t r){
    ...
    int fd = open((char *)(r.filename), O_APPEND, S_IRUSR);
    if(fd == -1){
        perror("append");
    ...
    FILE *fa  = fopen ((char *)(r.filename), "a");
    fprintf(fa, "\n %s \n", r.text);
